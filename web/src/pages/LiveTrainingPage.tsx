...
import { attemptsService, metricsService, workersService, stepsService, systemService, jobsService } from '../api';
import {
  MetricItemData,
...
export const LiveTrainingPage: React.FC = () => {
  ...
  const legacyStepsAdapter: any[] = steps.map((st) => {
    ...
    return {
      ...
      outputModelVersion: st.output_model_version ?? (st.input_model_version != null ? Number(st.input_model_version) + 1 : null),
      totalSampleCount: st.total_sample_count ?? 192,
      loss: detail?.metrics?.loss,
      accuracy: detail?.metrics?.accuracy,
      metrics: detail?.metrics,
      timings: {
        startedAt: timing?.started_at || (st.committed_at ? 'Committed' : 'In progress'),
        committedAt: timing?.committed_at || st.committed_at || undefined,
            ...
            {(() => {
              ...
              return (
                <div className="flex items-center gap-2 text-xs text-[#73737c] flex-wrap">
                  ...
                  <span>·</span>
                  <span className="font-mono text-[#a1a1a8]">Strategy: {trainingStrategy === 'strict_bsp' ? 'Strict BSP' : trainingStrategy}</span>
                </div>
              );
            })()}
          </div>
